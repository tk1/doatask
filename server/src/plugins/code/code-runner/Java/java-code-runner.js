...
const javaCommandCharacter = process.platform === 'win32' ? ';' : ':';
const codeCompilationCommand = 'javac';
const getCodeCompilationArguments = (codeFileName) => ['-cp', `.${javaCommandCharacter}gson-2.9.0.jar`, codeFileName];
const codeExecutionCommand = 'java';
const getCodeExecutionArguments = (codeFileNameWithoutExtension) => ['-cp', `.${javaCommandCharacter}gson-2.9.0.jar`, codeFileNameWithoutExtension];
